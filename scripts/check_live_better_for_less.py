...
def main() -> None:
    ...
    require("Quality-screened options" in lifestyle, "Lifestyle Abroad still advertises a price-only shortlist")
    require("qualityScreen(home, d)" in lifestyle and "r.quality.passes" in lifestyle, "Lifestyle Abroad shortlist must apply quality gates before ranking")
    require("country-systems-atlas.json" in lifestyle and "quality-of-life-supplement.json" in lifestyle, "Lifestyle Abroad quality sources are not loaded")
    require("a.required - b.required" not in lifestyle, "Lifestyle Abroad must not rank candidates by lowest price alone")
