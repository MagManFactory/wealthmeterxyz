#!/usr/bin/env python3
"""Generate Nano Banana Pro visuals for the 2026-04-03 WealthMeter longform article.

Usage:
  GOOGLE_API_KEY=... python3 scripts/generate_2026_04_03_wealthmeter_longform_images_nanobanana.py
  python3 scripts/generate_2026_04_03_wealthmeter_longform_images_nanobanana.py --api-key "<GOOGLE_API_KEY>"
"""
...
ARTIFACT_DIR = ROOT / "output" / "review" / "longform-pair-2026-04-03" / "wealthmeter-nanobanana"
...
PROMPTS: list[dict[str, str]] = [
    {
        "name": "rent-buy-reversal-cost-stack.png",
        "title": "Rent Buy Reversal Cost Stack",
        "prompt": (
            "Create a 16:9 editorial infographic titled "
            "'The Rent vs Buy Reversal'. "
            "Use two stacked columns labeled 'Rent' and 'Buy'. "
            "Under Rent show concise blocks: 'monthly rent', 'renter insurance', 'mobility preserved'. "
            "Under Buy show larger stacked blocks: 'mortgage interest', 'property tax', 'insurance', 'maintenance', 'transaction drag', 'down payment lock-up'. "
            "Across the bottom add a strip labeled 'what changed' with the notes 'higher rates', 'higher insurance', 'higher tax drag', 'mobility more valuable'. "
            "Footer note: 'Ownership still works, but the hurdle is higher.'"
        ),
    },
    {
        "name": "rent-buy-decision-matrix.png",
        "title": "Rent Buy Decision Matrix",
        "prompt": (
            "Create a 16:9 decision matrix titled "
            "'When Renting Beats Buying'. "
            "Use a 2x2 grid. Horizontal axis: 'expected stay short' to 'expected stay long'. "
            "Vertical axis: 'location needs unstable' to 'location needs stable'. "
            "Top-left quadrant label: 'rent' with short notes 'high mobility value', 'transaction costs hard to recover'. "
            "Top-right quadrant label: 'lean rent' with notes 'long stay possible, but uncertainty still high'. "
            "Bottom-left quadrant label: 'rent or delay purchase' with notes 'stable finances, short horizon'. "
            "Bottom-right quadrant label: 'buy can work' with notes 'long horizon', 'stable metro fit', 'repair reserve'. "
            "Add a footer strip reading 'time horizon decides more cases than ideology'."
        ),
    },
]
