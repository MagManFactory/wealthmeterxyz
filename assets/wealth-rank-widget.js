(function(){
  ...
  form.addEventListener("submit",eventObject=>{eventObject.preventDefault();if(!state.authorized)return;const raw=Number(document.getElementById("wealth-widget-value").value);const currency=document.getElementById("wealth-widget-currency").value;if(!Number.isFinite(raw)||raw<0)return;const result=position(raw*(FX[currency]||1));const rank=document.getElementById("wealth-widget-rank"),summary=document.getElementById("wealth-widget-summary");if(result.range){rank.textContent="#2.226B–#3.846B";summary.textContent="The source data groups values below US$10,000 into a broad 0–42.1st percentile band.";}else{rank.textContent=`#${result.rank.toLocaleString()}`;summary.textContent=`Approximately the ${result.percentile.toFixed(2)}th percentile — the top ${result.top.toFixed(result.top<.001?4:2)}% of adults in the global model.`;}document.getElementById("wealth-widget-result").classList.add("visible");if(!state.interactionSent){event("widget_interaction");state.interactionSent=true;}});
